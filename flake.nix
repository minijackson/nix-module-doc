{
  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      ...
      checks.x86_64-linux =
        let
          evalModules = modules: pkgs.lib.evalModules {
            modules = [
        ...
        in
        {
          simple-doc-options-md = (evalModules [
            simpleModule
            (self.lib.modules.doc-options-md params)
          ]).config.my.simple.module.outputs.doc-options-md;

          simple-manpage = (evalModules [
            simpleModule
            (self.lib.modules.doc-options-md params)
          ...
          ]).config.my.simple.module.outputs.manpage;

          advanced-manpage = (evalModules [
            simpleModule
            (self.lib.modules.doc-options-md params)
            (self.lib.modules.manpage params)
            {
          ...
          ]).config.my.simple.module.outputs.mdbook;
